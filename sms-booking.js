...
// Use already-registered mongoose models
const User = mongoose.model('User');
const Booking = mongoose.model('Booking');
const Machinery = mongoose.model('Machinery');
const Labour = mongoose.model('Labour');
...
function parseSMSCommand(smsText) {
    ...
    if (upper.startsWith('BOOK ')) {
        ...
        // Split around date
        const beforeDate = cleaned.substring(5, cleaned.indexOf(date)).trim();
        const afterDate = cleaned.substring(cleaned.indexOf(date) + date.length).trim();

        return {
            type: 'machinery',
            item: beforeDate.toLowerCase(),
            date: date,
            location: afterDate
        };
    }
...
async function handleSMSBooking(smsText, phoneNumber) {
    try {
        ...
        if (!parsed) {
            return {
                success: false,
                message: 'Invalid format.\nUse: BOOK TRACTOR YYYY-MM-DD LOCATION\nOr: LABOR SKILL QTY DATE'
            };
        }
...
BOOK tractor 2026-02-16 kalyani`
        ...
        if (!user) {
            user = await User.create({
                name: 'SMS User',
                email: `sms_${phoneNumber}@farmconnect.local`,
                password: 'sms-disabled',
                phone: phoneNumber,
                role: 'customer'
            });
...
Status: Pending confirmation`
        ...
        return {
            success: false,
            message: 'Server error while processing booking. Try again later.'
        };
    }
}
