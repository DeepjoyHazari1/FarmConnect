...
const bookingSchema = new mongoose.Schema({
    smsBookingId: { type: String, unique: true, sparse: true },
    customerId: { type: mongoose.Schema.Types.ObjectId, ref: 'User', required: true },
...
const smsBooking = require('./sms-booking'); 
...
// SMS Helper Function for Order Confirmation
async function sendOrderConfirmationSMS(orderData, customerPhone) {
    try {
        if (!customerPhone) return;
        
        const message = `✅ Order ${orderData.orderId} confirmed!\nAmount: ₹${orderData.total}\nItems: ${orderData.items.length}\nStatus: ${orderData.status}\nThank you for using FarmConnect!`;
...
app.post('/api/orders', async (req, res) => {
    try {
        ...
        if (customerInfo.phone) {
            sendOrderConfirmationSMS(exportOrder, customerInfo.phone);
        }
...
app.post('/api/sms/incoming', async (req, res) => {
    try {
        console.log('📱 SMS received from Twilio:', req.body);
        ...
        console.log(`✅ SMS processed: ${result.message}`);
        res.set('Content-Type', 'text/xml');
        res.send('<Response></Response>'); // Empty TwiML response
        
    } catch (error) {
        console.error('❌ SMS processing error:', error);
        res.status(500).send('<Response><Message>Error processing SMS</Message></Response>');
    }
});
...
app.post('/api/sms/send', express.json(), async (req, res) => {
    try {
        const { to, message, bookingId, orderId } = req.body;
...
// SMS Booking Status Check
app.get('/api/sms/booking/:bookingId', async (req, res) => {
    try {
        ...
        res.json({
            ...
            machinery: booking.machineryId,
            smsHistory: smsHistory
        });
        
    } catch (error) {
        console.error('❌ SMS booking status error:', error);
        res.status(500).json({ success: false, error: error.message });
    }
});
...
app.post('/api/sms/test', express.json(), async (req, res) => {
    try {
        ...
        const testMessage = `FarmConnect SMS Test ✅\n\nYour SMS system is working! You can book services via SMS:\n• BOOK [machinery] [date] [location]\n• LABOR [skill] [quantity] [date]\n\nReply with HELP for more info.`;
        ...
        res.status(500).json({ 
            success: false, 
            error: error.message,
            tip: 'Make sure your Twilio account has sufficient balance and the phone number is verified'
        });
    }
});

// Bulk SMS for notifications
app.post('/api/sms/bulk', express.json(), async (req, res) => {
    try {
        const { phoneNumbers, message, bookingIds } = req.body;
        ...
        for (const [index, phoneNumber] of phoneNumbers.entries()) {
            try {
                ...
                const smsRecord = new SMS({
                    ...
                    direction: 'outgoing',
                    status: 'sent',
                    bookingId: bookingIds ? bookingIds[index] : null
                });
                await smsRecord.save();
...
// SMS Command Help
app.get('/api/sms/help', (req, res) => {
    const helpMessage = {
        commands: [
            {
                command: "BOOK [machinery] [date] [location]",
                example: "BOOK tractor 2024-01-15 kalyani",
                description: "Book machinery for specific date"
            },
            {
                command: "LABOR [skill] [quantity] [date]",
                example: "LABOR planting 2 2024-01-15",
                description: "Book labour with specific skill"
            },
            {
                command: "STATUS [bookingId]",
                example: "STATUS SMS123456",
        ...
        available_machinery: ["tractor", "harvester", "plough", "rotavator", "sprayer", "drone"],
        available_skills: ["planting", "harvesting", "irrigation", "spraying", "pruning"],
        contact: "For assistance, call: +1-800-FARM-CONNECT"
    };
    
    res.json(helpMessage);
});

// SMS Booking Reminders (Scheduled Function)
...
// Schedule reminders (runs daily at 9 AM)
setInterval(sendBookingReminders, 24 * 60 * 60 * 1000); // 24 hours
...
const startServer = async () => {
    ...
    app.listen(5500, () => {
        console.log(`\n🚀 FarmConnect server running at http://localhost:5500`);
        ...
        console.log(`📱 SMS: Twilio integration enabled`);
        
        console.log(`\n📋 Available Pages:`);
        console.log(`- Home: http://localhost:${5500}/`);
        console.log(`- Dashboard: http://localhost:${5500}/dashboard`);
        console.log(`- Machinery: http://localhost:${5500}/machinery`);
        console.log(`- Labour: http://localhost:${5500}/labour`);
        console.log(`- Login: http://localhost:${5500}/login`);
        console.log(`- Register: http://localhost:${5500}/register`);
        
        console.log(`\n🔗 API Endpoints:`);
        console.log(`- Health Check: http://localhost:${5500}/api/health`);
        console.log(`- Database Info: http://localhost:${5500}/api/db-info`);
        console.log(`- Register: POST http://localhost:${5500}/api/register`);
        console.log(`- Login: POST http://localhost:${5500}/api/login`);
        
        console.log(`\n📱 SMS Features:`);
        console.log(`- SMS Incoming Webhook: POST http://localhost:${5500}/api/sms/incoming`);
        console.log(`- Send SMS: POST http://localhost:${5500}/api/sms/send`);
        console.log(`- SMS Test: POST http://localhost:${5500}/api/sms/test`);
        console.log(`- SMS History: GET http://localhost:${5500}/api/sms/history`);
        console.log(`- SMS Booking: POST http://localhost:${5500}/api/sms-booking`);
        ...
        // Start booking reminders after 10 seconds
        setTimeout(sendBookingReminders, 10000);
    });
};
